...
The present analysis was preregistered on the Open Science Framework \todo{add link}.
The preregistration was written in ignorance of the data to avoid any HARKING (hypothesizing after results known).\todo{add link to explain what harking is or why to preregister blindly}.
After knowledge of the data a large proportion of the planned analyses were deemed unfeasible due to limited variability of the data at hand.
...
\section{Deviation from the preregistration}
The preregistration lists a number of other constructs and scales that were planned for inclusion in the covariate network.
Upon closer inspection I decided to not include those variables/scales in the analysis.
Justification for this step is based in theory as well as methodology.
The construct of dating violence is based on the premise of a romantic relationship.
Consequently the scale was only assessed for participants who have been in a romantic relationship within 12 months prior to assessment.
A partial correlation network with items that are simultaneously indicators for the presence of a romantic relationship are likely 
not interpretable with regard to the construct they were intended to measure (dating violence). I hence decided to not include the CADRI scale in the analysis.
\todo{add justification for exclusion of violent perpetration items}.
...
As suggested by Epskamp \todo{add reference} the \texttt{sessionInfo()}
...
All random seeds are provided directly in the code.

This report was generated on $2020-05-04 15:27:17$ using the following computational environment and dependencies:
